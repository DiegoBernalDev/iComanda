...
Desarrollar una aplicación móvil y web que permita registrar pedidos, gestionar
mesas, pagos, menú, cocina, gastos, inventario y reportes financieros en tiempo
real para restaurantes pequeños y medianos.
...
\begin{enumerate}[leftmargin=1.8em]
  ...
  \item Registrar gastos operativos e inventario de materiales del restaurante.
  \item Generar reportes automáticos de ingresos brutos, netos, egresos y ranking
        de platos más vendidos.
  \item Implementar acceso diferenciado por rol: \texttt{mesero}, \texttt{admin} y
        \texttt{chef}, además de rutas públicas para clientes.
  \item Notificar a los meseros en tiempo real cuando un pedido está listo para
        ser entregado.
  \item Permitir al rol \texttt{chef} visualizar pedidos de cocina, marcarlos como
        listos y gestionar platos agotados.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Administradores:} responsables de la gestión del menú, gastos y
        reportes financieros, usuarios, mesas e inventario.
  \item \textbf{Chef/cocina:} responsable de visualizar pedidos en preparación,
        marcar órdenes listas y controlar platos agotados.
  \item \textbf{Clientes en mesa:} usuarios ocasionales que interactúan mediante
        una ruta pública por QR, sin necesidad de registro ni descarga de app.
\end{itemize}
...
\begin{longtable}{>{\bfseries}p{1.5cm} p{11cm}}
  ...
  RF-07 & Acceso diferenciado: mesero, admin y chef con rutas y permisos propios. \\[3pt]
  RF-08 & Notificaciones en tiempo real al mesero cuando el pedido pasa a estado \texttt{lista}. \\[3pt]
  ...
  RF-11 & El cliente puede escanear un QR por mesa para abrir una vista pública y ver el menú con disponibilidad real. \\[3pt]
  RF-12 & El cliente puede llamar al mesero desde su dispositivo, generando una notificación en tiempo real al mesero asignado. \\[3pt]
  RF-13 & El chef puede ver pedidos activos y marcarlos como \texttt{lista}. \\[3pt]
  RF-14 & El chef puede marcar platos como agotados o disponibles. \\[3pt]
  RF-15 & El administrador puede registrar materiales y movimientos de inventario. \\[3pt]
  RF-16 & El administrador puede consultar ventas por mesero y liberar mesas después del pago confirmado. \\
  \bottomrule
...
\begin{longtable}{>{\bfseries}p{2cm} p{11cm}}
  ...
  RNF-07 & Base de código preparada para builds y actualizaciones mediante el ecosistema Expo. \\[3pt]
...
administradores, meseros, chef/cocina y clientes.
...
\hline
Estado & Completada & Asignado & Equipo \\
\hline
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \cellcolor{lightgray!30}\textbf{Descripción} \par \vspace{1pt}
...
\hline
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item La estructura de carpetas sigue la convención \texttt{app/} con grupos \texttt{login}, \texttt{(mesero)} y \texttt{(admin)}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item \texttt{lib/supabase-public.ts} expone un cliente separado para carta digital y vista pública de mesa.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item Las migraciones crean las tablas principales: \texttt{profiles}, \texttt{restaurants}, \texttt{tables}, \texttt{menu\_items}, \texttt{orders}, \texttt{order\_items}, \texttt{expenses}, \texttt{table\_calls}, \texttt{materials} y \texttt{material\_stock\_movements}.
    \item Todos los enums del sistema están definidos: \texttt{rol\_usuario}, \texttt{estado\_orden}, \texttt{metodo\_pago} y \texttt{material\_movement\_type}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El índice excluye órdenes con \texttt{estado IN ('entregada', 'cancelada')}.
    \item Intentar insertar una segunda orden activa en la misma mesa falla con error de constraint a nivel de base de datos.
    \item Una vez que la orden pasa a \texttt{entregada} o \texttt{cancelada}, la misma mesa acepta una nueva orden sin errores.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \cellcolor{lightgray!30}\textbf{Descripción} \par \vspace{1pt}
  Como sistema, quiero que \texttt{closed\_at} se asigne automáticamente al pasar una orden a estado \texttt{entregada} para no depender del cliente y garantizar la integridad del historial financiero.
} \\
\hline
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Al ejecutar \texttt{UPDATE orders SET estado='entregada'}, el campo \texttt{closed\_at} se puebla automáticamente con \texttt{now()}.
    \item El trigger es de tipo \texttt{BEFORE UPDATE} para que el valor quede disponible en la misma transacción.
    \item Si la orden ya tenía \texttt{estado='entregada'}, una actualización posterior no sobreescribe \texttt{closed\_at}.
    \item Las órdenes con \texttt{estado='cancelada'} no reciben valor en \texttt{closed\_at}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Al autenticarse con credenciales correctas, el sistema carga el perfil desde \texttt{profiles} y redirige a \texttt{(mesero)}, \texttt{(admin)} o \texttt{(chef)} según el rol.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \cellcolor{lightgray!30}\textbf{Descripción} \par \vspace{1pt}
  Como sistema, quiero que cada usuario acceda únicamente a las rutas correspondientes a su rol para respetar el acceso diferenciado definido en los requisitos funcionales.
} \\
\hline
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item Un usuario con \texttt{rol=mesero} que intenta acceder a rutas \texttt{(admin)} es redirigido a la pantalla de login.
    \item Un usuario con \texttt{rol=admin} que intenta acceder a rutas \texttt{(mesero)} es redirigido al área de admin.
    \item Un usuario con \texttt{rol=chef} es redirigido al área de cocina \texttt{(chef)}.
    ...
    \item El guard utiliza el perfil cargado desde \texttt{profiles} en el contexto de autenticación.
    \item Los guards están implementados en \texttt{app/\_layout.tsx} y redirigen según el grupo de rutas.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \cellcolor{lightgray!30}\textbf{Descripción} \par \vspace{1pt}
  Como sistema, quiero que el rol de la tabla \texttt{profiles} se mantenga consistente con Supabase Auth para que el control de acceso pueda resolverse de forma confiable.
} \\
\hline
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item El trigger \texttt{trg\_sync\_role} es de tipo \texttt{AFTER INSERT OR UPDATE OF rol} sobre la tabla \texttt{profiles}.
    \item Al modificar el campo \texttt{rol} en \texttt{profiles}, el metadata interno de \texttt{auth.users} se actualiza automáticamente.
    \item La función del trigger usa \texttt{SECURITY DEFINER} para poder escribir en el esquema \texttt{auth}.
    \item El nuevo rol queda disponible para la aplicación después de refrescar la sesión o recargar el perfil.
    \item La sincronización no afecta otros campos de metadata existentes.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item La política \texttt{mesero\_own\_orders} restringe \texttt{SELECT} al \texttt{mesero\_id = auth.uid()}.
    \item La política \texttt{mesero\_insert\_orders} permite \texttt{INSERT} solo si el rol en el JWT es \texttt{mesero}.
    \item La política \texttt{mesero\_update\_own\_orders} permite \texttt{UPDATE} solo sobre las órdenes propias del mesero.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item La política \texttt{public\_menu\_read} permite \texttt{SELECT} en \texttt{menu\_items} únicamente sobre ítems con \texttt{disponible=true} sin autenticación.
    \item La política \texttt{cliente\_full\_menu\_read} permite \texttt{SELECT} del menú completo a usuarios con rol \texttt{cliente}.
    \item La política \texttt{cliente\_insert\_table\_call} permite \texttt{INSERT} en \texttt{table\_calls} solo si el \texttt{table\_id} del JWT coincide con el de la fila.
    \item La política \texttt{mesero\_table\_calls} permite al mesero ver y actualizar llamadas de las mesas con órdenes activas que le pertenecen.
    \item La inserción pública de llamadas queda limitada a los datos de mesa/restaurante validados desde la URL.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item La lista diferencia visualmente entre roles \texttt{mesero} y \texttt{admin}.
    ...
    \item Los datos se obtienen desde la tabla \texttt{profiles}, no directamente desde \texttt{auth.users}.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El usuario se crea en Supabase Auth y simultáneamente en la tabla \texttt{profiles} con el mismo \texttt{id}.
    \item El rol queda registrado en \texttt{profiles.rol} y disponible para el contexto de autenticación.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Al desactivar, el campo \texttt{activo=false} se actualiza en la tabla \texttt{profiles}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item La URL pública del CDN se guarda en el campo \texttt{imagen\_url} del ítem creado.
    ...
    \item El ítem se crea con \texttt{disponible=true} por defecto salvo que el admin lo deshabilite explícitamente.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El cambio de precio no afecta las órdenes anteriores, ya que el precio se guarda como snapshot en \texttt{order\_items.precio\_unitario}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Al desactivar, el campo \texttt{disponible=false} se actualiza inmediatamente en la base de datos.
    \item Un ítem con \texttt{disponible=false} no aparece en la carta pública ni en la vista del mesero al tomar pedidos.
    \item Un ítem con \texttt{disponible=false} sí aparece en la vista del menú del cliente en mesa, marcado visualmente como no disponible.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Si el ítem tiene \texttt{order\_items} asociados, el sistema lo desactiva (\texttt{disponible=false}) en lugar de eliminarlo, preservando la integridad del historial.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item La vista muestra únicamente ítems con \texttt{disponible=true}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El campo \texttt{precio\_unitario} de cada \texttt{order\_item} se guarda como snapshot del precio actual, no como referencia dinámica.
    \item El \texttt{total} de la orden se calcula automáticamente como suma de los subtotales.
    \item La orden se crea con \texttt{estado='activa'} y \texttt{pago\_confirmado=false} por defecto cuando requiere confirmación posterior.
    ...
    \item El mesero queda como \texttt{mesero\_id} de la orden recién creada.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El estado actual de la orden (\texttt{activa}, \texttt{lista}, \texttt{entregada}) es visible de forma destacada.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item La opción de agregar ítems solo está disponible cuando la orden tiene \texttt{estado='activa'}.
    ...
    \item No es posible agregar ítems a una orden con \texttt{estado='lista'} o superior.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item La acción de marcar como entregada solo está disponible cuando la orden tiene \texttt{estado='lista'}.
    \item Al confirmar, el estado transiciona de \texttt{lista} a \texttt{entregada}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item La cancelación solo es posible cuando la orden tiene \texttt{estado='activa'}.
    \item Al confirmar, el estado transiciona a \texttt{cancelada}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item El formulario de nueva orden incluye un selector con las opciones \texttt{efectivo} y \texttt{qr}.
    \item El método de pago seleccionado se guarda en \texttt{orders.metodo\_pago}.
    ...
    \item Para órdenes con \texttt{metodo\_pago='efectivo'} o \texttt{tarjeta}, el pago puede confirmarse al entregar la orden.
    \item Para órdenes con \texttt{metodo\_pago='qr'}, \texttt{pago\_confirmado} permanece en \texttt{false} hasta que el admin lo confirme.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item La lista filtra órdenes con \texttt{metodo\_pago='qr'} y \texttt{pago\_confirmado=false}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item Al confirmar, \texttt{orders.pago\_confirmado} cambia de \texttt{false} a \texttt{true}.
    \item La confirmación actualiza la misma fila de \texttt{orders} y registra \texttt{paid\_at}.
    ...
    \item El campo \texttt{paid\_at} en \texttt{orders} se registra con la marca de tiempo exacta de la confirmación.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El QR se muestra solo cuando la orden tiene \texttt{metodo\_pago='qr'}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El gasto se guarda en la tabla \texttt{expenses} con el \texttt{created\_by} del admin en sesión.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El filtro se aplica sobre el campo \texttt{fecha} de la tabla \texttt{expenses}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item El estado de cada mesa se deriva del campo \texttt{estado} de la orden activa asociada.
    \item Las mesas sin orden activa se muestran con estado Libre.
    \item Esta pantalla es la pantalla principal del rol \texttt{mesero} al iniciar sesión.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item El hook \texttt{useTables} se suscribe al canal \texttt{orders} de Supabase Realtime filtrado por \texttt{mesero\_id}.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item Al recibir un evento \texttt{UPDATE} en \texttt{orders} donde \texttt{estado='lista'}, se dispara una alerta visible en pantalla.
...
% HU-39
\subsubsection*{HU-39 — Mostrar botón ``Llamar al mesero'' en la vista pública de mesa}
\begin{table}[H]
\centering
\small
\renewcommand{\arraystretch}{1.18}
...
\multicolumn{3}{l|}{\color{white}\bfseries Mostrar botón ``Llamar al mesero'' en la vista pública de mesa} \\
\hline
Estado & Completada & Asignado & Equipo \\
\hline
Esfuerzo & 2 puntos & Prioridad & Alta \\
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Al presionarlo, se inserta un registro en \texttt{table\_calls} con el \texttt{table\_id} y \texttt{restaurant\_id} validados desde la ruta pública.
    \item La inserción usa el cliente público de Supabase y no requiere cuenta del cliente.
    ...
    \item Si la mesa o el restaurante no son válidos, la pantalla muestra error y no permite registrar llamadas.
    \item El cliente no puede acceder a funcionalidades internas de mesero, admin ni chef.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item Al marcar como atendida, se ejecuta \texttt{UPDATE table\_calls SET atendida=true, atendida\_at=now()}.
    ...
    \item La operación solo la puede ejecutar el mesero con la política RLS \texttt{mesero\_table\_calls} activa.
    \item El campo \texttt{atendida\_at} queda registrado con la marca de tiempo exacta de la atención.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Solo se muestran ítems con \texttt{disponible=true}, aplicando la política \texttt{public\_menu\_read}.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Vista pública de cliente en mesa (web)}

% HU-45
\subsubsection*{HU-45 — Abrir vista pública al escanear el QR de la mesa}
\begin{table}[H]
\centering
\small
\renewcommand{\arraystretch}{1.18}
...
\multicolumn{3}{l|}{\color{white}\bfseries Abrir vista pública al escanear el QR de la mesa} \\
\hline
Estado & Completada & Asignado & Equipo \\
\hline
Esfuerzo & 3 puntos & Prioridad & Alta \\
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \cellcolor{lightgray!30}\textbf{Descripción} \par \vspace{1pt}
  Como cliente en mesa, quiero que al escanear el QR de mi mesa se abra una página pública vinculada a esa mesa sin que tenga que registrarme ni instalar una aplicación.
} \\
\hline
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item Al acceder a \texttt{/table/[slug]/[tableId]}, el sistema valida que el restaurante y la mesa existan.
    \item El cliente no necesita ingresar ningún dato ni crear una cuenta para usar la vista.
    \item Un \texttt{tableId} inexistente muestra un mensaje de error descriptivo al cliente.
    \item La ruta pública no tiene acceso a ninguna funcionalidad de mesero, administrador ni chef.
    \item Ejemplo local validado: \texttt{http://localhost:8081/table/jimmy/b3383250-5517-4772-8bda-549bf27d44e1}.
  \end{enumerate}
} \\
\hline
\end{tabularx}
\caption{HU-45: Vista pública del cliente en mesa}
\end{table}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  \textbf{Criterios de aceptación} \vspace{1pt}
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    \item La política \texttt{cliente\_full\_menu\_read} permite la lectura de todos los ítems del menú, incluyendo no disponibles.
    ...
    \item La vista carga desde la ruta pública de mesa y no requiere inicio de sesión del cliente.
  \end{enumerate}
} \\
\hline
\end{tabularx}
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Solo contabiliza órdenes con \texttt{estado='entregada'} y \texttt{pago\_confirmado='confirmed'}.
    \item Los gastos se filtran por \texttt{fecha} dentro del rango indicado.
...
\multicolumn{4}{|p{\dimexpr\textwidth-2\tabcolsep-2\arrayrulewidth}|}{%
  ...
  \begin{enumerate}[leftmargin=1.5em, itemsep=0pt, parsep=2pt, topsep=0pt]
    ...
    \item Retorna una tabla con columnas \texttt{menu\_item\_id}, \texttt{nombre}, \texttt{total\_qty} y \texttt{total\_revenue}.
    \item Los resultados se ordenan por \texttt{total\_qty} de forma descendente.
    \item Solo incluye ítems de órdenes con \texttt{estado='entregada'} y \texttt{pago\_confirmado='confirmed'}.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Historias complementarias implementadas}

Las siguientes historias documentan funcionalidades reales agregadas al alcance
final del proyecto para reflejar el código implementado en los módulos de cocina,
inventario, reportes y cierre operativo.

\begin{longtable}{|>{\bfseries}p{1.5cm}|p{5.2cm}|p{2.2cm}|p{5cm}|}
\hline
\rowcolor{huorange}
\color{white}\bfseries ID & \color{white}\bfseries Historia & \color{white}\bfseries Estado & \color{white}\bfseries Criterios principales \\
\hline
\endhead

HU-52 & Implementar rol chef/cocina & Completada & El rol \texttt{chef} existe en \texttt{profiles.rol}; el guard redirige a \texttt{(chef)}; cocina tiene navegación propia. \\
\hline
HU-53 & Panel de pedidos para cocina & Completada & El chef visualiza órdenes \texttt{activa} y \texttt{lista}; se muestran mesa, hora e ítems; los datos se actualizan con Realtime. \\
\hline
HU-54 & Marcar pedido como listo desde cocina & Completada & Cocina actualiza \texttt{orders.estado='lista'}; el mesero recibe aviso visual; la orden cambia de estado sin recargar. \\
\hline
HU-55 & Marcar plato agotado o disponible desde cocina & Completada & El chef alterna \texttt{menu\_items.agotado}; se usa la RPC \texttt{set\_menu\_item\_sold\_out}; el menú refleja el cambio. \\
\hline
HU-56 & Crear módulo de inventario de materiales & Completada & El admin accede a \texttt{inventario}; puede crear/editar/desactivar materiales con unidad y stock mínimo. \\
\hline
HU-57 & Registrar reposiciones y consumos de inventario & Completada & El admin registra movimientos \texttt{reposicion} y \texttt{consumo}; se guardan cantidad, motivo, notas y usuario. \\
\hline
HU-58 & Visualizar materiales con stock bajo & Completada & La pantalla calcula el stock actual a partir de movimientos y resalta materiales con stock menor o igual al mínimo. \\
\hline
HU-59 & Reporte de ventas por mesero & Completada & La función \texttt{get\_waiter\_sales} calcula número de órdenes y total vendido por mesero en un rango de fechas. \\
\hline
HU-60 & Liberar mesa después de pago confirmado & Completada & La función \texttt{clear\_table\_after\_payment} permite limpiar la mesa cuando la orden entregada tiene pago confirmado. \\
\hline
HU-61 & Generar comprobante de orden entregada & Completada & El sistema genera un comprobante HTML con detalle de orden, mesa, ítems, total y método de pago. \\
\hline
HU-62 & Registrar fecha efectiva de pago & Completada & Las órdenes confirmadas guardan \texttt{paid\_at}; los reportes se calculan por fecha real de pago. \\
\hline
HU-63 & Generar snapshot server-side del estado de mesas & Completada & La RPC \texttt{get\_waiter\_table\_state\_snapshot} consolida mesa, orden activa, última orden entregada y llamada pendiente. \\
\hline
\caption{Historias de usuario complementarias implementadas}
\end{longtable}

...
del sistema (mesero, administrador, chef y cliente) y los servicios principales
...
la visualización de la carta digital y la vista pública del cliente en mesa mediante
el uso de códigos QR. Estas rutas usan acceso público controlado mediante
...
La implementación real del proyecto utiliza \textbf{Expo SDK 54}, \textbf{Expo Router 6},
\textbf{React 19}, \textbf{React Native 0.81}, \textbf{TypeScript} y
\textbf{supabase-js v2}. A nivel de aplicación se definieron grupos de rutas para
\texttt{(admin)}, \texttt{(mesero)} y \texttt{(chef)}, además de rutas públicas para
la carta digital y la sesión de mesa del cliente.

El backend administrado se apoya en Supabase PostgreSQL para persistencia,
Supabase Auth para sesiones, Supabase Storage para imágenes, Supabase Realtime
para sincronización operativa y funciones RPC para procesos transaccionales como
creación de órdenes, reportes, ranking de platos, ventas por mesero, limpieza de
mesa e inventario.
...
móvil utilizada por el mesero, el administrador y cocina. Estos componentes
permiten gestionar pedidos, visualizar el estado de mesas, administrar el menú,
controlar inventario, marcar pedidos listos y consultar reportes financieros.
...
Los componentes Mesero UI, Admin UI y Chef UI representan las interfaces de usuario
...
y la vista del cliente en mesa. Estas funcionalidades permiten que el cliente
...
\caption{Componentes de la carta digital y vista pública de cliente en mesa}
\end{figure}
...
mesas, pedidos, gastos, inventario y reportes. El servicio Auth permite gestionar sesiones
de usuario mediante tokens JWT para roles internos. Las rutas de carta digital y
mesa se consultan mediante acceso público limitado por RLS.
...
La tabla \texttt{profiles} almacena los usuarios del sistema con sus respectivos
...
Las tablas \texttt{materials} y \texttt{material\_stock\_movements} permiten controlar
materiales, reposiciones, consumos y stock mínimo.
...
\begin{tabular}{ll}
  ...
  Pendiente              & \texttt{estado = 'activa'} \\
  Listo para entregar    & \texttt{estado = 'lista'} \\
  Entregado              & \texttt{estado = 'entregada'} \\
  Cliente llama          & Registro en \texttt{table\_calls} con \texttt{atendida = false} \\
  \bottomrule
\end{tabular}
\end{center}
